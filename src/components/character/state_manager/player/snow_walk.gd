...
func on_enter(_prev_state):
	self.host.play_animation("snow-walk")
...
func do_process(_delta):
	if not self.enabled:
		return
	if not self.host.current_animation == "snow-walk":
		self.host.play_animation("snow-walk")
	...
	if self.host.get_node("Skin").animation_data.frames[self.host.get_node("Skin").current_frame].duration == 500:
		self.host.velocity = Vector2.ZERO 
