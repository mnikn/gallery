...
@export_dir var character_texture_path = ""
...
func start_animation(new_animation = null):
	...
	var texture_path = self.character_texture_path + "/" + animation_name + ".png"
	var animation_data = FileUtils.read_json_file(self.character_texture_path + "/" + animation_name + ".json")
	$Sprite.start(animation_name, texture_path, animation_data)
...
func _physics_process(delta):
	...
	if x_direction and $Sprite.animation_data:
		if $Sprite.animation_data.frames[$Sprite.current_frame].duration == 500 and self.walk_type == "snow-walk":
